...
import at.jku.dke.task_app.hierarchical_clustering.data.entities.BinarySearchTaskGroup;
import at.jku.dke.task_app.hierarchical_clustering.data.repositories.BinarySearchTaskGroupRepository;
import at.jku.dke.task_app.hierarchical_clustering.dto.ModifyBinarySearchTaskGroupDto;
...
class TaskGroupControllerTest {
    ...
    void createShouldReturnCreated() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.CRUD_API_KEY)
            .contentType(ContentType.JSON)
            .body(new ModifyTaskGroupDto<>("hierarchical-clustering", TaskStatus.APPROVED, new ModifyBinarySearchTaskGroupDto(1, 5)))
            // WHEN
            .when()
    ...
    void updateShouldReturnOk() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.CRUD_API_KEY)
            .contentType(ContentType.JSON)
            .body(new ModifyTaskGroupDto<>("hierarchical-clustering", TaskStatus.APPROVED, new ModifyBinarySearchTaskGroupDto(1, 10)))
            // WHEN
            .when()
